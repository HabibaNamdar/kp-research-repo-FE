...
const FeaturedPublication: React.FC = () => {
  ...
  return (
    <>
      <NavBar />
      <section className="featured-publication-section py-5">
        <div className="container">
          {/* Title & Authors */}
          <h2 className="fw-bold mb-3">{publication.title}</h2>
          <p className="text-muted mb-2">
            {publication.author} &nbsp;|&nbsp; Published <strong>{publication.year}</strong>
          </p>
          <p className="text-muted">
            Field: <span className="fw-semibold">{publication.category}</span>
          </p>
          ...
          {/* Keywords / Tags */}
          <h5 className="fw-bold mt-5 mb-3">Keywords</h5>
          <div className="mb-4">
          ...
          {/* Abstract */}
          <h5 className="fw-bold mb-3">Abstract</h5>
          <p className="text-muted">{publication.desc}</p>

          {/* References */}
          {publication.references && (
            <>
              <h5 className="fw-bold mt-4 mb-3">References</h5>
              {publication.references.map((ref: string, i: number) => (
                <p key={i} className="text-muted">{ref}</p>
              ))}
            </>
          )}
          ...
          <div className="d-flex flex-wrap gap-3 my-5">
            {[1, 2].map((_, i) => (
              <button key={i} className="btn btn-outline-primary d-flex align-items-center gap-2">
                <FaLock />
          ...
          {/* Recommended Section */}
          <h5 className="fw-bold mt-5 mb-3">You may also like</h5>
          <div className="row g-3">
            {[
              { title: "AI in Healthcare Data Analysis", author: "James Smith", field: "Machine Learning" },
              { title: "Machine Learning in Disease Prediction", author: "Rachel Green", field: "AI Research" },
              { title: "Neural Network Models for Radiology", author: "Michael Chang", field: "Deep Learning" },
            ].map((rec, i) => (
              <div key={i} className="col-md-4">
                <div className="card border-0 shadow-sm p-3">
                  <h6 className="fw-bold">{rec.title}</h6>
                  <p className="text-muted small mb-1">{rec.author}</p>
                  <p className="text-primary small">{rec.field}</p>
                </div>
              </div>
            ))}
          </div>

          {/* <button className="btn btn-secondary mt-5" onClick={() => navigate("/browse")}>
            ← Back to Browse
          </button> */}
        </div>
      </section>
